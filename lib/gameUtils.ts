import { BoundingBox, ImageData, LocationKey } from '@/types/game';

export type { LocationKey };

export const boundingBoxVN: BoundingBox = {
...
export const locationNames: Record<LocationKey, string> = {
  'HN': 'HA NOI',
...
export async function getRandomMapillaryImage(choiceLocation: LocationKey): Promise<ImageData | null> {
  ...
  try {
    ...
    if (data.data && data.data.length > 0) {
      const panoImages = data.data.filter((image: any) => image.is_pano);
